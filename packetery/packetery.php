<?php
/*
* Copyright 2012 Zásilkovna, s.r.o.
*/
...
class Packetery extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->description = $this->l('Offers your customers the option to choose pick-up point in Packetery network, and export orders to Packetery system.');
    ...
    private static function transportMethod()
    {
        ...
        if(ini_get('allow_url_fopen')) {
            $have_url_fopen = true;
            $have_https_fopen = ($have_url_fopen && extension_loaded('openssl') && function_exists('stream_context_create'));
        }
        // Disabled - more trouble than it's worth
        //if($have_curl_ssl) return 'curls';
        //if($have_https_fopen) return 'fopens';
        if($have_curl) return 'curl';
        if($have_url_fopen) return 'fopen';
        return false;
    }
    ...
    public function configuration_errors(&$error = null)
    {
        ...
        $fn = _PS_MODULE_DIR_."packetery/write-test.js";
        @touch($fn);
        if(!is_writable($fn)) {
            $error[] = $this->l('The Packetery module folder must be writable for the branch selection to work properly.');
            $have_error = true;
        }

        if(!self::transportMethod()) {
            $error[] = $this->l('No way to access Packetery API is available on the web server: please allow CURL module or allow_url_fopen setting.');
            $have_error = true;
        }
        ...
        elseif(!$error) {
            if($this->fetch($test) != 1) {
                ...
                $data = json_decode($this->fetch("http://www.zasilkovna.cz/api/$key/version-check-prestashop?my=" . self::VERSION));
                if(self::compare_versions($data->version, self::VERSION) > 0) {
                    global $cookie;
    ...
    public function install()
    {
        ...
        if($have_old_table) {
            ...
            $db->execute('insert into `'._DB_PREFIX_.'packetery_order`(`' . implode('`, `', $fields) . '`) select * from `'._DB_PREFIX_.'packetery_order_old`');
    ...
    public function uninstall()
    {
        ...
        // remove admin tab
        $db = Db::getInstance();
        if($tab_id = $db->getValue('select id_tab from `' . _DB_PREFIX_ . 'tab` where class_name="AdminOrderPacketery"')) {
        ...
        $db->execute('update `' . _DB_PREFIX_.'carrier` set deleted=1 where external_module_name="packetery" or id_carrier in (select id_carrier from `'._DB_PREFIX_.'packetery_carrier`)');
    ...
    public function c_add_carrier_post()
    {
        ...
        $carrier->name = $_POST['packetery_carrier_name'];
        $carrier->active = true;
    ...
    private function c_configuration()
    {
        ...
        $html .= "<div class='margin-form'><input type='text' name='packetery_api_key' value='" . htmlspecialchars(Configuration::get('PACKETERY_API_KEY'), ENT_QUOTES) . "' /></div>";
        $html .= "<div class='clear'></div>";

        $html .= "<label>" . $this->l('E-shop domain') . ": </label>";
        $html .= "<div class='margin-form'><input type='text' name='packetery_eshop_domain' value='" . htmlspecialchars(Configuration::get('PACKETERY_ESHOP_DOMAIN'), ENT_QUOTES) . "' /><p>" . $this->l('If you\'re using one Packetery account for multiple e-shops, enter the domain of current one here, so that your customers are properly informed about what package they are receiving.') . "</p></div>";
        $html .= "<div class='clear'></div>";

        $html .= "<div class='margin-form'><input class='button' type='submit' value='" . htmlspecialchars($this->l('Save'), ENT_QUOTES) . "'  /></div>";
    ...
    {
        return array(1 => $this->l('Selection box only'), $this->l('Selection box with map'), $this->l('Selection box with direct details display'));
    }
    ...
    private function c_add_carrier()
    {
        ...
        $html .= "<div class='margin-form'><input type='text' name='packetery_carrier_name' size='41' value='" . htmlspecialchars($this->l('Personal pick-up – Packetery'), ENT_QUOTES) . "' /></div>";
        $html .= "<div class='clear'></div>";
        ...
        foreach(Language::getLanguages(false) as $language) {
            ...
            $html .= '<div id="delay_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == 1 ? 'block' : 'none').'; float: left;"><input type="text" size="41" maxlength="128" name="delay_'.$language['id_lang'].'" value="'.htmlspecialchars($delay[$language['iso_code']] ? $delay[$language['iso_code']] : $delay['en'], ENT_QUOTES).'" /></div>';
        }
        $html .= $this->displayFlags(Language::getLanguages(false), $def_lang, 'delay', 'delay', true);
        $html .= '<p class="clear"></p></div>';
        $html .= "<div class='clear'></div>";
        $html .= "<script type='text/javascript'> changeLanguage('delay', 'delay', $def_lang, '$def_lang_code'); </script>";
        ...
        $html .= "</select>";
        $html .= "<p class='clear'>" . $this->l('You can select one or more countries by using the Ctrl key. Only branches in selected countries will be shown in this shipping method – you can e.g. set different price based on country.') . "</p>";
        $html .= "</div>";
        $html .= "<div class='clear'></div>";
        ...
        $html .= "</select>";
        $html .= "<input id='packetery-preview-button' type='button' class='button' value='" . htmlspecialchars($this->l('Show preview'), ENT_QUOTES) . "' />";
        $html .= "<p class='clear'>" . $this->l('Here you can change the display of branch list in shopping cart. Preview will only work if you have correct API key entered.') . "</p>";
        $html .= "</div>";
        $html .= "<div class='clear'></div>";

        $html .= "<label for='packetery_carrier_is_cod'>" . $this->l('Is COD') . ": </label>";
        $html .= "<div class='margin-form'><input type='checkbox' id='packetery_carrier_is_cod' name='packetery_carrier_is_cod' value='1'><p>" . $this->l('When exporting order with this carrier, the order total will be put as COD.') . "</div>";
        $html .= "<div class='clear'></div>";
        
        $html .= "<label>" . $this->l('Install Logo') . ": </label>";
        $html .= "<div class='margin-form'>";
        foreach(array("" => $this->l('No'), "cz" => "<img style='vertical-align: top; ' src='"._MODULE_DIR_."packetery/logo-cz.jpg'>", "sk" => "<img style='vertical-align: top; ' src='"._MODULE_DIR_."packetery/logo-sk.jpg'>") as $k => $v) {
            $html .= "<input type='radio' name='packetery_carrier_logo' value='$k' id='packetery_carrier_logo_$k'><label for='packetery_carrier_logo_$k' style='width: auto; height: auto; float: none; display: inline; '>$v</label> &nbsp; &nbsp; ";
        }
        $html .= "</div>";
        $html .= "<div class='clear'></div>";
        
        if(!$this->configuration_errors()) {
            $html .= "<script type='text/javascript' src='//www.zasilkovna.cz/api/" . Configuration::get('PACKETERY_API_KEY') . "/branch.js?sync_load=1&amp;prestashop=1'></script>";
            $html .= '
<script type="text/javascript">
  window.packetery.jQuery(function() {
    ...
    private function c_list_carriers()
    {
        ...
        if($list = $db->executeS('select c.id_carrier, c.name, pc.country, pc.list_type, pc.is_cod from `'._DB_PREFIX_.'carrier` c join `'._DB_PREFIX_.'packetery_carrier` pc on(pc.id_carrier=c.id_carrier) where c.deleted=0')) {
            ...
            $html .= "<tr><th>" . $this->l('Carrier Name') . "</th><th>" . $this->l('Countries') . "</th><th>" . $this->l('Selection Type') . "</th><th>" . $this->l('Is COD') . "</th><th>" . $this->l('Action') . "</th></tr>";
            $list_types = $this->list_types();
            foreach($list as $carrier) {
                $html .= "<tr><td>$carrier[name]</td><td>$carrier[country]</td><td>" . $list_types[$carrier['list_type']] . "</td><td>" . ($carrier['is_cod'] == 1 ? $this->l('Yes') : $this->l('No')) . "</td><td><form method='post'><input type='hidden' name='packetery_remove_carrier' value='$carrier[id_carrier]'><input type='submit' class='button' value='" . htmlspecialchars($this->l('Remove'), ENT_QUOTES) . "'></form></td></tr>";
            }
            $html .= "</table>";
    ...
    private function c_list_payments_post()
    {
        if(isset($_POST['packetery_payment_module']) && $_POST['packetery_payment_module']) {
            ...
            if($db->getValue('select 1 from `' . _DB_PREFIX_.'packetery_payment` where module_name="' . pSQL($_POST['packetery_payment_module']) . '"') == 1) {
                ...
                $db->execute('insert into `' . _DB_PREFIX_.'packetery_payment` set is_cod=' . ((int) $_POST['packetery_payment_is_cod']) . ', module_name="' . pSQL($_POST['packetery_payment_module']) . '"');
            }
        }
    }
    ...
    private function c_list_payments()
    {
        ...
        foreach($modules as $module) {
            ...
            $is_cod = ($db->getValue('select is_cod from `'._DB_PREFIX_.'packetery_payment` where module_name="' . pSQL($module['name']) . '"') == 1);
            $html .= "<tr><td>$instance->displayName</td><td>" . ($is_cod == 1 ? $this->l('Yes') : $this->l('No')) . "</td><td><form method='post'><input type='hidden' name='packetery_payment_module' value='" . htmlspecialchars($module['name'], ENT_QUOTES) . "' /><input type='hidden' name='packetery_payment_is_cod' value='" . (1 - $is_cod) . "' /><input type='submit' class='button' value='" . htmlspecialchars($is_cod ? $this->l('Clear COD setting') : $this->l('Set COD setting'), ENT_QUOTES) . "'></form></td></tr>";
        }
        $html .= "</table>";
    ...
    private function c_list_address_delivery_carriers_post()
    {
        ...
        foreach($data as $id_carrier => $attr) {
            if($attr['id_branch']) {
                ...
                $db->execute('insert into `' . _DB_PREFIX_.'packetery_address_delivery`(id_carrier, id_branch, name_branch, currency_branch, is_cod) values(' . ((int) $id_carrier) . ', ' . ((int) $attr['id_branch']) . ', "' . pSQL($a->name) . '", "' . pSQL($a->currency) . '", ' . ((int) $attr['is_cod']) . ') on duplicate key update id_branch=' . ((int) $attr['id_branch']) . ', is_cod=' . ((int) $attr['is_cod']) . ', name_branch="' . pSQL($a->name) . '", currency_branch="' . pSQL($a->currency) . '"');
            }
            else {
                $db->execute('delete from `' . _DB_PREFIX_.'packetery_address_delivery` where id_carrier=' . ((int) $id_carrier));
            }
        }
    }
    ...
    private function c_list_address_delivery_carriers()
    {
        ...
        $html .= "<table class='table' cellspacing='0'>";
        $html .= "<tr><th>" . $this->l('Carrier') . "</th><th>" . $this->l('Is Address Delivery via Packetery') . "</th><th>" . $this->l('Is COD') . "</th></tr>";
        ...
        foreach($carriers as $carrier) {
            $html .= "<tr><td>" . ($carrier['name'] != "0" ? $carrier['name'] : Configuration::get('PS_SHOP_NAME')) . "</td><td><select name='data[$carrier[id_carrier]][id_branch]'>";
            foreach((array('' => (object) array('name' => '–– ' . strToLower($this->l('No')) . ' ––')) + self::address_deliveries()) as $k => $v) {
                $html .= "<option value='$k'" . ($carrier['id_branch'] == $k ? " selected" : "") . ">$v->name</option>\n";
            }
            $html .= "</select></td><td><select name='data[$carrier[id_carrier]][is_cod]'>";
            foreach(array($this->l('No'), $this->l('Yes')) as $k => $v) {
                $html .= "<option value='$k'" . ($carrier['is_cod'] == $k ? " selected" : "") . ">$v</option>\n";
            }
            $html .= "</select></td></tr>";
        }
        $html .= "</table>";
        $html .= "<input type='submit' class='button' value='" . htmlspecialchars($this->l('Save settings'), ENT_QUOTES) . "'>";
        $html .= "<p>" . $this->l('Changes will not affect existing orders, only those created after your changes.') . "</p>";
        $html .= "</fieldset>";
        return $html;
    }
    ...
    public function getContent()
    {
        ...
        $this->c_configuration_post();
        $this->c_add_carrier_post();
        $this->c_list_carriers_post();
        $this->c_list_payments_post();
        $this->c_list_address_delivery_carriers_post();
        ...
        $html .= "<br>";
        $html .= $this->c_configuration();
        $html .= "<br>";
        $html .= $this->c_list_carriers();
        $html .= "<br>";
        $html .= $this->c_add_carrier();
        $html .= "<br>";
        $html .= $this->c_list_address_delivery_carriers();
        $html .= "<br>";
        $html .= $this->c_list_payments();
        
        return $html;
    }

    static $is_before_carrier = false;
    ...
    function hookExtraCarrier($params)
    {
        ...
        if($db->getValue('select 1 from `' . _DB_PREFIX_.'hook` where name in ("beforeCarrier", "displayBeforeCarrier")') == 1 && !self::$is_before_carrier) {
            return "";
        }

        $carrier_data = array();
        foreach($db->executeS('select pc.id_carrier, pc.country, pc.list_type from `'._DB_PREFIX_.'packetery_carrier` pc join `'._DB_PREFIX_.'carrier` c using(id_carrier) where c.deleted=0') as $carrier) {
            $carrier_data[$carrier['id_carrier']] = array('country' => $carrier['country'], 'list_type' => $carrier['list_type']);
        }
        
        $selected_id = $db->getValue('select id_branch from `'._DB_PREFIX_.'packetery_order` where id_cart=' . ((int) $params['cart']->id));
  ...
  window.packetery.jQuery(function() {
      ...
      var carrier_data = ' . json_encode($carrier_data) . ';
      var last_ajax = null;
      ...
      var update_id_carriers = function(reinitialize) { id_carriers = $(id_carriers_selector); on_updated_id_carriers(reinitialize); };
      ...
      if(original_updateAddressSelection) {
          window.updateAddressSelection = function() {
              ...
              update_id_carriers(true);
              els.each(function() {
                  find_select_place.call(id_carriers.filter("[value=\"" + $(this).data("delivery_option_value") + "\"]")[0]).after(this);
              });
          }
      }
      if(original_updatePaymentMethods) {
          window.updatePaymentMethods = function() {
              if(!submit_now_allowed) {
                  arguments[0].HOOK_PAYMENT = ' . json_encode('<p class="warning">'.Tools::displayError('Error: please choose a carrier').'</p>') . ';
              }
              try {
      ...
      if(original_paymentModuleConfirm) {
          ...
          window.paymentModuleConfirm = function() {
              if(!submit_now_allowed) {
                  alert(' . json_encode($this->l('Please select pick-up point.')) . ');
                  return false;
              }
      ...
      var id_carrier_init = function() {
          ...
          if(carrier.is("tr")) e = $("<tr><td colspan=\'" + carrier.closest("table").find("tr:first").find("th,td").size() + "\'>" + please_select + "<div></div></td></tr>");
    ...
    {
        $db = DB::getInstance();
        if($packetery_carrier = $db->getRow('select is_cod from `'._DB_PREFIX_.'packetery_carrier` where id_carrier=' . ((int) $params['order']->id_carrier))) {
            // branch
        }
        elseif($packetery_carrier = $db->getRow('select is_cod, id_branch, name_branch, currency_branch from `'._DB_PREFIX_.'packetery_address_delivery` where id_carrier=' . ((int) $params['order']->id_carrier))) {
            // address
            $db->execute('insert ignore into `'._DB_PREFIX_.'packetery_order` set id_cart=' . ((int) $params['cart']->id));
            $db->execute('update `'._DB_PREFIX_.'packetery_order` set id_branch=' . ((int) $packetery_carrier['id_branch']) . ', name_branch="' . pSQL($packetery_carrier['name_branch']) . '", currency_branch="' . pSQL($packetery_carrier['currency_branch']) . '" where id_cart=' . ((int) $params['cart']->id));
        }
        else {
            return;
        }
        $db->execute('update `'._DB_PREFIX_.'packetery_order` set id_order=' . ((int) $params['order']->id) . ' where id_cart=' . ((int) $params['cart']->id));

        $carrier_is_cod = ($packetery_carrier['is_cod'] == 1);
        $payment_is_cod = ($db->getValue('select is_cod from `'._DB_PREFIX_.'packetery_payment` where module_name="' . pSQL($params['order']->module) . '"') == 1);
        if($carrier_is_cod || $payment_is_cod) {
            $db->execute('update `'._DB_PREFIX_.'packetery_order` set is_cod=1 where id_order=' . ((int) $params['order']->id));
        }
    }

    public function hookAdminOrder($params)
    {
        if(!($res = Db::getInstance()->getRow('SELECT o.name_branch FROM `'._DB_PREFIX_.'packetery_order` o WHERE o.id_order = '. ((int) $params['id_order'])))) {
            return "";
        }
        
        return "<p>".sprintf($this->l('Selected packetery branch: %s'), "<strong>".$res['name_branch']."</strong>")."</p>";
    }
    
    public function hookOrderDetailDisplayed($params)
    {
        if(!($res = Db::getInstance()->getRow('SELECT o.name_branch FROM `'._DB_PREFIX_.'packetery_order` o WHERE o.id_order = '. ((int) $params['order']->id)))) {
            return;
        }
    ...
    public function hookHeader($params)
    {
        ...
        $this->ensureUpdatedAPI();
        return '<script type="text/javascript" src="'._MODULE_DIR_.'packetery/api.js"></script>';
    }
    
    private function fetch($url)
    {
        $transportMethod = self::transportMethod();
        if(substr($transportMethod, -1) == 's') {
    ...
    */
    private function ensureUpdatedAPI()
    {
        $key = Configuration::get('PACKETERY_API_KEY');
        $files = array(
            _PS_MODULE_DIR_."packetery/api.js" =>
                "http://www.zasilkovna.cz/api/$key/branch.js?lib_path="._MODULE_DIR_."packetery&sync_load=1&prestashop=1",
        ...
        foreach($files as $local => $remote) {
            if(date("d.m.Y", @filemtime($local)) != date("d.m.Y") && (!file_exists($local) || date("H") >= 1)) {
                if($this->configuration_errors() || strlen($data = $this->fetch($remote)) <= 1024) {
    ...
    public static function address_deliveries()
    {
        ...
        if(function_exists("simplexml_load_file") && file_exists($fn)) {
            ...
            if(function_exists('mb_convert_encoding')) {
                $fn = create_function('$a,$b', 'return strcmp(mb_convert_encoding($a->name, "ascii", "utf-8"), mb_convert_encoding($b->name, "ascii", "utf-8"));');
            }
            else {
                $fn = create_function('$a,$b', 'return strcmp($a->name, $b->name);');
            }
            uasort($res, $fn);
        }
        return $res;
    ...
    {
        $db = Db::getInstance();
        $is_packetery_carrier = ($db->getValue('select 1 from `'._DB_PREFIX_.'packetery_carrier` where id_carrier=' . ((int) $params['cart']->id_carrier)) == 1);
        $has_selected_branch = ($db->getValue('select id_branch from `'._DB_PREFIX_.'packetery_order` where id_cart=' . ((int) $params['cart']->id)) > 0);
    ...
    public function hookBackOfficeTop($params) {
        ...
        if($cookie->packetery_seen_warning < 3) {
            ...
            if(!$this->configuration_errors($errors) && count($errors) > 0) {
                return "<div style='float: right; width: 400px; font-weight: bold; color: red'>" . $errors[0] . "</div>";
            }
        }
    }
}
