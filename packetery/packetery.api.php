...
class PacketeryApi
{
    ...
    public static function createPacket($order)
    {
        ...
        if ($is_packetery_ad) {
            ...
            $packetery_ad_zip = $address_delivery->postcode;
            ...
            $packet_attributes['houseNumber'] = $packetery_house_number;
        }
    ...
    /*BRANCHES*/
    public static function updateBranchCron()
    {
        $last_update = Packeteryclass::getConfigValueByOption('LAST_BRANCHES_UPDATE');
        if ($last_update == '') {
            $last_update_number = 0;
        } else {
            $last_update_number = date("d", $last_update);
        }
        $now_time = time();
        $now_time_number = date("d", $now_time);

        if ($now_time_number != $last_update_number) {
            $hour = (int)date("H");
            if ($hour >= 2) {
    ...
    public static function getApiKey($apiPassword = false)
    {
        if (!$apiPassword) {
            $apiPassword = Packeteryclass::getConfigValueByOption('Apipass');
        }
        $apiKey = Tools::substr($apiPassword, 0, 16);
        return $apiKey;
    }
    ...
    public static function updateBranchListAjax()
    {
        ...
        if (!$result) {
            ...
            echo json_encode(array(2, $result));
        }
    }
    ...
    public static function updateBranchList($apiPassword = false)
    {
        ...
        $ad_branches = self::parseBranches($branch_ad_url, 1);
        if ((!$branches) && (!$ad_branches) && ($countBranches = self::countBranches())) {
            Packeteryclass::updateSetting(5, time());
            return false;
        } else {
            return ($branches ? $branches: '').($ad_branches ? $ad_branches: '').($countBranches ? $countBranches: '');
        }
    }

    public static function parseBranches($branch_url, $is_ad = 0)
    {
    ...
    public static function countBranchesAjax()
    {
        ...
        if ($lastUpdateUnix != '') {
            ...
            $lastBranchesUpdate = $date->format('Y-m-d H:i:s');
        }
    ...
    public static function dropBranchList($is_ad = 0)
    {
        $sql = 'DELETE FROM '._DB_PREFIX_.'packetery_branch
                WHERE is_ad = '.(int)$is_ad;
    ...
    public static function addBranch($branch, $is_ad = 0)
    {
    ...
    /*WIDGET*/
    public static function getCountriesList($id_carrier = false)
    {
        ...
        $out = array();
        if ($id_carrier) {
            $carrier_row = Packeteryclass::getPacketeryCarrierRow($id_carrier);
            $countries_str = $carrier_row['country'];
            $countries = explode(',', $countries_str);
            foreach ($result as $res) {
                if (in_array($res['country'], $countries)) {
                    $out[] = $res;
                }
            }
        } else {
    ...
    public static function widgetSaveOrderBranch()
    {
        ...
        $branch = self::getBranchObject($id_branch);
